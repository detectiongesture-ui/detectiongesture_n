...
def run_auto_labeler(unlabelled_dir, output_labeled_dir, confidence_threshold=0.75):
    """
    ...
    with torch.no_grad():
        for filename in raw_files:
            ...
            # Extract basic tracking matrix
            matrix = extract_gesture_matrix_from_image(img_path)
            
            # Grab the raw relative features from the first row of our sequence timeline
            # Features 0-33 are flat coordinates. Let's isolate wrist and torso metrics
            # Left wrist Y is index 19, Right wrist Y is index 21 in flattened array
            lw_y = matrix[0][19]
            rw_y = matrix[0][21]
            ...
            # Negative numbers mean higher up on screen. Positive numbers mean lower down.
            ...
            if assigned_label == 'ThumbDown' and highest_wrist_y < 0.2:
                # If the hand is up high near the torso/neck, override ThumbDown to neckpain!
                assigned_label = 'neckpain'
                best_prob = 0.92 # Upgrade structural score confidence
                
            # Rule 2: Fix the Headache/Shoulder swallowed by ThumbUp Trap
            elif assigned_label == 'ThumbUp':
                if highest_wrist_y < -0.3: # Hand is very high up near ears/head
                    assigned_label = 'headache'
                elif -0.3 <= highest_wrist_y < -0.05: # Hand is at upper shoulder collar line
                    assigned_label = 'ShoulderPain'

            # Define validation gate rules
            is_valid = True
